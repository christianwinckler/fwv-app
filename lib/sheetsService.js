...
export async function getDetalle() {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GASTOS_SHEET_ID,
    range: 'Detalle!A:J',
  })
...
export async function updateParametros(rows) {
  ...
  const filasPrevias = (existingRes.data.values || []).length
  ...
  const filasNuevas = rowsFinales.length - filasPrevias
  if (filasNuevas > 0 && filasPrevias > 0) {
    const metaRes = await sheets.spreadsheets.get({
      spreadsheetId: process.env.GASTOS_SHEET_ID,
      fields: 'sheets.properties',
    })
    const paramSheet = metaRes.data.sheets.find(
      s => s.properties.title === 'Parámetros'
    )
    const paramSheetId = paramSheet.properties.sheetId
    const filaRefIndex = filasPrevias + 1 // última fila con datos antes del append (1-based, +1 por header)

    const requests = []
    for (let i = 0; i < filasNuevas; i++) {
      const destinoIndex = filasPrevias + 2 + i // 1-based row de la nueva fila
      requests.push({
        copyPaste: {
          source: {
            sheetId: paramSheetId,
            startRowIndex: filaRefIndex - 1,
            endRowIndex: filaRefIndex,
            startColumnIndex: 0,
            endColumnIndex: 6,
          },
          destination: {
            sheetId: paramSheetId,
            startRowIndex: destinoIndex - 1,
            endRowIndex: destinoIndex,
            startColumnIndex: 0,
            endColumnIndex: 6,
          },
...
export async function copiarFormatoUltimaFilaParametros(nuevaFila) {
  const sheets = await getSheets()
  ...
  const filaReferencia = nuevaFila - 1
...
export async function getMontoInicialTC() {
  ...
  const b4 = parsearMonto(res.data.values?.[3]?.[0])
  return { b1, b2, total: b1 + b2, montoInicialAhorros: b4 }
}

export async function renameCatEnPresupuesto(oldCat, newCat) {
  const sheets = await getSheets()
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GASTOS_SHEET_ID,
    range: 'Presupuesto!C:C',
  })
  const values = res.data.values || []
  const updates = []
  values.forEach((row, idx) => {
    if (idx === 0) return
    if ((row[0] || '').trim() === oldCat.trim()) {
      updates.push({ range: `Presupuesto!C${idx + 1}`, values: [[newCat]] })
    }
  })
...
export async function renameSubcatEnPresupuesto(oldSub, newSub, newCat) {
  const sheets = await getSheets()
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GASTOS_SHEET_ID,
    range: 'Presupuesto!B:C',
  })
  const values = res.data.values || []
  const updates = []
  values.forEach((row, idx) => {
    if (idx === 0) return
    if ((row[0] || '').trim() === oldSub.trim()) {
      updates.push({ range: `Presupuesto!B${idx + 1}`, values: [[newSub]] })
      if (newCat) {
        updates.push({ range: `Presupuesto!C${idx + 1}`, values: [[newCat]] })
      }
    }
